...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=isaacloft0906/multi-server:$SHA
kubectl set image deployments/client-deployment client=isaacloft0906/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=isaacloft0906/multi-worker:$SHA
